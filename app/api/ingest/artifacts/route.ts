...
export async function POST(request: Request) {
  ...
  const expectedKey = `history/${MODEL_VERSION}/${scanId}/${market}/${assetType}.parquet`;
  if (!scanId || !/^[\w-]{8,80}$/.test(scanId) || objectKey !== expectedKey || !idempotencyKey || idempotencyKey.length > 160 || !market || !MARKETS.includes(market as typeof MARKETS[number]) || !assetType || !["STOCK", "ETF"].includes(assetType)) return jsonError("Invalid artifact metadata", 400);
