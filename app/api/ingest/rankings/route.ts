...
export async function POST(request: Request) {
  ...
  let payload: { scan?: ScanSummary; records?: RankedSecurity[]; batchIndex?: number; batchCount?: number };
  ...
  if (records.some((item) => !item.instrumentId || !MARKETS.includes(item.market) || item.modelVersion !== MODEL_VERSION)) return jsonError("Invalid ranking record", 400);
