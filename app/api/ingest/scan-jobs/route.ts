...
import { CANDIDATE_MODEL_VERSION, MARKETS, isSupportedModelVersion } from "@/lib/types";
...
export async function POST(request: Request) {
  ...
  let body: { market?: string; assetType?: string; trigger?: string; modelVersion?: string };
  ...
  try {
    const modelVersion = String(body.modelVersion ?? CANDIDATE_MODEL_VERSION);
    ...
    if (body.trigger === "SCHEDULED" && MARKETS.includes(market as typeof MARKETS[number])) {
      ...
      const active = await db.prepare("SELECT analysis_captured_at FROM active_scan_outputs WHERE model_version=? AND market=?").bind(modelVersion, market).all<{ analysis_captured_at:string }>();
