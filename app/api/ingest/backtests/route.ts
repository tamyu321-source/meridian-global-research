...
export async function POST(request: Request) {
  ...
  let payload: { modelVersion?: string; configHash?:string; validationStatus?:string; survivorshipBias?:boolean; generatedAt?: string; markets?: Record<string, { metrics?: Record<string, unknown>; trades?: unknown[] }> };
  try { payload = JSON.parse(body) as typeof payload; } catch { return jsonError("Invalid JSON", 400); }
  if (payload.modelVersion !== MODEL_VERSION || payload.validationStatus !== "PROVISIONAL_BACKTEST" || !payload.survivorshipBias || !payload.generatedAt || !payload.markets) return jsonError("Invalid provisional v2 backtest artifact", 400);
