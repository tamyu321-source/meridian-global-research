...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (modelVersion === CANDIDATE_MODEL_VERSION) return Response.json({
      rankings:[], meta:{ mode:"SHADOW", validationStatus:"SHADOW", backtestStatus:"NOT_STARTED", formalEligible:false, primaryFeed:"public_sources", discovery:"awaiting_candidate_full_analysis", ibkrConnected:false, persistence:"available", markets, errors:[], generatedAt:new Date().toISOString(), scan:null, modelVersion },
    }, { headers:{ "Cache-Control":"private, no-store" } });
    ...
    return Response.json({
      rankings: ranked, meta: { mode: "SHADOW", validationStatus:"SHADOW", backtestStatus:"NOT_STARTED", formalEligible:false, primaryFeed: "public_sources", discovery: "limited_live_fallback_watch_only", ibkrConnected: false, persistence, markets, errors: errors.slice(0, 8), generatedAt: new Date().toISOString(), scan:null, modelVersion:ACTIVE_MODEL_VERSION },
    }, { headers: { "Cache-Control": "private, no-store" } });
