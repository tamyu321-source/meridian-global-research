...
export async function GET() {
  ...
  try {
    ...
    return Response.json({ ibkr: { connected: false, reason: "public_data_only" }, storage: { d1: true, r2: Boolean(runtimeEnv().MARKET_ARCHIVE), artifacts:Number(artifacts?.count ?? 0), artifactBytes:Number(artifacts?.bytes ?? 0) }, model:{ modelVersion:MODEL_VERSION, validationStatus:"SHADOW", formalEligible:false }, shadowValidation:{ days:Number(shadow?.days ?? 0), completenessPct:Number(shadow?.completeness ?? 0), freshnessPct:Number(shadow?.freshness ?? 0), consistencyPct:Number(shadow?.consistency ?? 0), incidents:Number(shadow?.incidents ?? 0), requiredDays:30 }, markets, fullScan:scan ? { id:scan.id, status:scan.status, completedAt:scan.completed_at, analyzedCount:Number(scan.analyzed_count), failedCount:Number(scan.failed_count), fallbackCount:Number(scan.fallback_count), qualityGatePassed:Boolean(scan.quality_gate_passed), sourceConflicts:Number(scan.source_conflicts), corporateActionAnomalies:Number(scan.corporate_action_anomalies), configHash:scan.config_hash, coverage:JSON.parse(String(scan.coverage_json ?? "{}")) } : null, generatedAt: new Date().toISOString() });
  } catch {
