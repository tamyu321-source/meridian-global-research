...
import type { Locale, MarketCode, PriceBar, RankedSecurity, RiskPlanId } from "@/lib/types";
...
export function MeridianApp({ view, instrumentId }: { view: AppView; instrumentId?: string }) {
  ...
  const loadRankings = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/rankings?market=${market}&assetType=${assetType}&riskPlan=${riskPlan}&modelVersion=${encodeURIComponent(modelVersion)}`, { cache: "no-store" });
  ...
  }, [market, assetType, riskPlan, modelVersion, locale, x]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLoading(true);
    fetch(`/api/securities/${encodeURIComponent(instrumentId)}?riskPlan=${riskPlan}&modelVersion=${encodeURIComponent(modelVersion)}`, { cache: "no-store" }).then(async (response) => {
      const result = await response.json() as { security?: RankedSecurity; bars?:PriceBar[] } & ApiErrorPayload;
      if (!response.ok || !result.security) throw new Error(apiErrorText(locale,result));
      setSelected(result.security); setSecurityBars(result.bars ?? []);
    }).catch((caught) => setError(caught instanceof Error ? caught.message : x("errorGeneric"))).finally(() => setLoading(false));
  }, [view, instrumentId, riskPlan, modelVersion, locale, x]);

  useEffect(() => {
    if (view === "security" || !selected?.instrumentId) return;
    const controller = new AbortController();
    fetch(`/api/securities/${encodeURIComponent(selected.instrumentId)}?riskPlan=${riskPlan}&modelVersion=${encodeURIComponent(modelVersion)}`, { cache:"no-store", signal:controller.signal })
      .then(async response => response.ok ? await response.json() as { bars?:PriceBar[] } : { bars:[] })
      .then(result => setSecurityBars(result.bars ?? [])).catch(() => undefined);
    return () => controller.abort();
  }, [view, selected?.instrumentId, riskPlan, modelVersion]);
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="side-nav" aria-label={x("mainNavigation")}>
        ...
        <div className="model-stamp"><span>{x("model")}</span><strong>{modelVersion}</strong><small>{x("holdingPeriod")}</small></div>
      </aside>
      <main className="workspace">
        {(["dashboard", "scanner", "signals", "security"] as AppView[]).includes(view) && <>
          ...
          {view !== "security" && <section className="control-deck">
            ...
            <Control label={x("modelTrack")}><button className={modelVersion === ACTIVE_MODEL_VERSION ? "active" : ""} onClick={() => setModelVersion(ACTIVE_MODEL_VERSION)}>{x("activeModel")}</button><button className={modelVersion === CANDIDATE_MODEL_VERSION ? "active" : ""} onClick={() => setModelVersion(CANDIDATE_MODEL_VERSION)}>{x("candidateModel")}</button></Control>
          </section>}
          {view !== "security" && modelVersion === CANDIDATE_MODEL_VERSION && <div className="candidate-lock-banner">{x("candidateLocked")}</div>}
          ...
          {!loading && !error && view !== "security" && <section className="research-layout">
            <div className="ranking-panel"><div className="panel-title"><strong>{payload?.rankings.length ?? 0} {x("ranked")} · {payload?.meta.scan?.analyzedCount?.toLocaleString(locale) ?? x("limited")} {x("analyzed")}</strong><span>{payload?.meta.generatedAt ? new Date(payload.meta.generatedAt).toLocaleString(locale) : "—"}</span></div>
              ...
              </button>)}</div> : <div className="empty-state">{modelVersion === CANDIDATE_MODEL_VERSION ? x("candidateAwaiting") : t.noData}</div>}
            </div>
            {selectedRank && <SecurityPanel security={selectedRank} bars={securityBars} locale={locale} t={t} quantity={quantity} setQuantity={setQuantity} onPaperBuy={paperBuy}/>}
          </section>}
          {!loading && view === "security" && selectedRank && <SecurityPanel security={selectedRank} bars={securityBars} locale={locale} t={t} quantity={quantity} setQuantity={setQuantity} onPaperBuy={paperBuy} standalone/>}
        </>}
...
function SecurityPanel({ security, bars, locale, t, quantity, setQuantity, onPaperBuy, standalone=false }: { security: RankedSecurity; bars:PriceBar[]; locale: Locale; t: typeof words[Locale]; quantity:number; setQuantity:(value:number)=>void; onPaperBuy:()=>void; standalone?:boolean }) {
  ...
  const candidateLocked=security.modelVersion===CANDIDATE_MODEL_VERSION&&security.marketProfileStatus!=="ACTIVE_SHADOW";
  return <aside className={`security-panel ${standalone ? "standalone" : ""}`}>
    ...
    <div className="risk-limit-grid"><div><span>{t.quality}</span><strong>{security.dataQuality?.completenessPct ?? 0}%</strong></div><div><span>{t.sources}</span><strong>{security.dataQuality?.sourceCount ?? 0}</strong></div><div><span>{t.bucket}</span><strong>{security.selection?.bucketRank ?? 0} / {security.selection?.buyLimit ?? 0} BUY</strong></div><div><span>{tx(locale,"model")}</span><strong>{security.assetModel}</strong></div>{security.marketProfileId&&<><div><span>{tx(locale,"modelProfile")}</span><strong>{security.marketProfileId}</strong></div><div><span>{tx(locale,"profileStatus")}</span><strong>{codeText(locale,security.marketProfileStatus)}</strong></div><div><span>{tx(locale,"strategyFamily")}</span><strong>{codeText(locale,security.strategyFamily)}</strong></div><div><span>{tx(locale,"gatePreset")}</span><strong>{codeText(locale,security.gatePreset)}</strong></div></>}</div>
    ...
    {candidateLocked&&<div className="candidate-paper-lock">{tx(locale,"candidatePaperLocked")}</div>}
    <div className="paper-action"><label>{t.qty}<input type="number" min="1" step="1" value={quantity} onChange={(event)=>setQuantity(Math.max(1,Number(event.target.value)))}/></label><button onClick={onPaperBuy} disabled={candidateLocked || security.action!=="BUY" || security.tradePlanState === "REANALYSIS_REQUIRED"}>{t.paperBuy}</button></div>
  </aside>;
}

function PriceChart({bars,security,locale}:{bars:PriceBar[];security:RankedSecurity;locale:Locale}) {
  const chart=useMemo(()=>{
    const daily=bars.slice(-1260),closes=daily.map(item=>item.adjClose??item.close),average=(end:number,period:number)=>end+1>=period?closes.slice(end-period+1,end+1).reduce((sum,value)=>sum+value,0)/period:null;
    const weekly=[] as Array<{timestamp:number;open:number;high:number;low:number;close:number;ma20:number|null;ma50:number|null;ma200:number|null}>;
    for(let start=0;start<daily.length;start+=5){const group=daily.slice(start,start+5),end=start+group.length-1;if(!group.length)continue;weekly.push({timestamp:group[group.length-1].timestamp,open:group[0].open,high:Math.max(...group.map(item=>item.high)),low:Math.min(...group.map(item=>item.low)),close:group[group.length-1].close,ma20:average(end,20),ma50:average(end,50),ma200:average(end,200)});}
    return weekly;
  },[bars]);
  if(chart.length<2)return null;
  const width=900,height=330,pad={left:48,right:14,top:18,bottom:28},plotW=width-pad.left-pad.right,plotH=height-pad.top-pad.bottom;
  const levels=[security.tradePlan.entryLow,security.tradePlan.entryHigh,security.tradePlan.stop,security.tradePlan.target1,security.tradePlan.target2,security.setupMetrics?.breakoutLevel??0].filter(value=>value>0);
  const min=Math.min(...chart.map(item=>item.low),...levels)*.985,max=Math.max(...chart.map(item=>item.high),...levels)*1.015,range=Math.max(.0001,max-min);
  const x=(index:number)=>pad.left+(index/(chart.length-1))*plotW,y=(value:number)=>pad.top+(max-value)/range*plotH,candle=Math.max(1,Math.min(4,plotW/chart.length*.55));
  const path=(key:"ma20"|"ma50"|"ma200")=>chart.map((item,index)=>item[key]==null?null:`${index===0||chart[index-1][key]==null?"M":"L"}${x(index).toFixed(1)},${y(Number(item[key])).toFixed(1)}`).filter(Boolean).join(" ");
  const line=(value:number,color:string,label:string,dash="4 3")=><g key={label}><line x1={pad.left} x2={width-pad.right} y1={y(value)} y2={y(value)} stroke={color} strokeWidth="1" strokeDasharray={dash}/><text x={width-pad.right-3} y={y(value)-3} textAnchor="end" fill={color} fontSize="9">{label} {value.toLocaleString(locale)}</text></g>;
  return <div className="price-chart"><h3>{tx(locale,"chart5y")}</h3><svg role="img" aria-label={`${security.symbol} ${tx(locale,"chart5y")}`} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none"><rect x={pad.left} y={pad.top} width={plotW} height={plotH} fill="#faf8f2"/>{chart.map((item,index)=>{const up=item.close>=item.open,color=up?"#3d6841":"#b44f3c";return <g key={item.timestamp}><line x1={x(index)} x2={x(index)} y1={y(item.high)} y2={y(item.low)} stroke={color} strokeWidth="1"/><rect x={x(index)-candle/2} y={y(Math.max(item.open,item.close))} width={candle} height={Math.max(1,Math.abs(y(item.open)-y(item.close)))} fill={color}/></g>})}<path d={path("ma20")} fill="none" stroke="#c07a34" strokeWidth="1.4"/><path d={path("ma50")} fill="none" stroke="#3f718d" strokeWidth="1.4"/><path d={path("ma200")} fill="none" stroke="#7b547c" strokeWidth="1.4"/>{security.setupMetrics?.breakoutLevel?line(security.setupMetrics.breakoutLevel,"#6d6a3d","B"):null}{line(security.tradePlan.entryLow,"#3d6841","E")}{line(security.tradePlan.stop,"#b44f3c","S")}{line(security.tradePlan.target1,"#3f718d","T1")}{line(security.tradePlan.target2,"#7b547c","T2")}<text x={pad.left} y={height-8} fontSize="9" fill="#777">{new Date(chart[0].timestamp*1000).toLocaleDateString(locale)}</text><text x={width-pad.right} y={height-8} textAnchor="end" fontSize="9" fill="#777">{new Date(chart[chart.length-1].timestamp*1000).toLocaleDateString(locale)}</text></svg><div className="chart-legend"><span className="ma20">SMA20</span><span className="ma50">SMA50</span><span className="ma200">SMA200</span><span>B · {tx(locale,"breakoutLevel")}</span><span>E/S/T · {tx(locale,"entryQuality")}</span></div></div>;
}

function PortfolioView({ locale, title, setup }: { locale:Locale; title:string; setup:string }) {
  type RiskPolicyView={riskBudgetPct:number;maxWeightPct:number;maxSectorPct:number;drawdownBreakerPct:number;enabledMarkets:MarketCode[];marketLimits:Record<MarketCode,number>;allowMinimumLotException:boolean};
  type PortfolioPayload={portfolio?:Record<string,unknown>|null;positions?:Array<Record<string,unknown>>;orders?:Array<Record<string,unknown>>;summary?:Record<string,unknown>|null;marketRules?:Array<Record<string,unknown>>;riskPolicy?:RiskPolicyView};
  ...
  const riskPlan=String(data.portfolio.risk_plan??"capital_first") as RiskPlanId,template=RISK_PLANS[riskPlan]??RISK_PLANS.capital_first;
  const limits=data.riskPolicy??{riskBudgetPct:template.riskBudgetPct,maxWeightPct:template.maxWeightPct,maxSectorPct:template.maxSectorPct,drawdownBreakerPct:template.drawdownBreakerPct,enabledMarkets:[...MARKETS],marketLimits:Object.fromEntries(MARKETS.map(item=>[item,template.maxMarketPct])) as Record<MarketCode,number>,allowMinimumLotException:true};
  ...
  return <PageSection title={title} eyebrow={x("portfolioEyebrow")}>
    ...
    <h3 className="section-label">{x("riskLimits")}</h3><div className="risk-limit-grid"><div><span>{x("riskPlan")}</span><strong>{riskPlanName(locale,riskPlan)}</strong></div><div><span>{x("singleTradeRisk")}</span><strong>{limits.riskBudgetPct}%</strong></div><div><span>{x("maxPosition")}</span><strong>{limits.maxWeightPct}%</strong></div><div><span>{x("maxSector")}</span><strong>{limits.maxSectorPct}%</strong></div><div><span>{x("drawdownBreaker")}</span><strong>{limits.drawdownBreakerPct}%</strong></div>{limits.enabledMarkets.map(item=><div key={item}><span>{x("marketLimit",{market:item})}</span><strong>{limits.marketLimits[item]}%</strong></div>)}</div><p className="risk-limit-note">{limits.allowMinimumLotException?x("minimumLotRule"):x("investmentMarketsNote")}</p>
...
function BacktestView({ locale, title }: { locale:Locale; title:string }) {
  const [data,setData]=useState<Record<string,unknown>|null>(null),[profiles,setProfiles]=useState<Array<Record<string,unknown>>>([]); useEffect(()=>{Promise.all([fetch(`/api/backtests/${CANDIDATE_MODEL_VERSION}`,{cache:"no-store"}).then(async r=>await r.json() as Record<string,unknown>),fetch("/api/model-profiles",{cache:"no-store"}).then(async r=>await r.json() as {profiles?:Array<Record<string,unknown>>})]).then(([backtest,profileData])=>{setData(backtest);setProfiles(profileData.profiles??[]);});},[]);
  ...
  return <PageSection title={title} eyebrow={tx(locale,"backtestEyebrow")}><div className="activation-banner"><span>{tx(locale,"formalLocked")} · {CANDIDATE_MODEL_VERSION}</span><strong>{data?codeText(locale,data.status):tx(locale,"loading")}</strong><p>{tx(locale,"candidateLocked")} {tFor(locale).provisional}</p></div>{acceptance&&<section className="metric-grid"><Metric label={tx(locale,"minimumTrades")} value={acceptance.minimumTrades}/><Metric label={tx(locale,"profitFactor")} value={`≥ ${acceptance.profitFactor}`}/><Metric label={tx(locale,"sharpe")} value={`≥ ${acceptance.sharpe}`}/><Metric label={tx(locale,"shadowDays")} value={acceptance.shadowTradingDays}/></section>}<h3 className="section-label">{tx(locale,"modelProfile")}</h3><div className="market-health-grid profile-grid">{profiles.map(row=><div key={String(row.profileId)}><span>{String(row.market)} · {codeText(locale,row.assetType)}</span><strong>{codeText(locale,row.status)}</strong><small>{codeText(locale,row.strategyFamily)} · {codeText(locale,row.gatePreset)}<br/>{tx(locale,"shadowDays")}: {String(row.shadowDays??0)} / 30<br/>{String(row.configHash??"").slice(0,12)}</small></div>)}</div><h3 className="section-label">{tx(locale,"backtestEyebrow")}</h3><div className="market-health-grid">{markets?.map(row=>{const metrics=(row.metrics??{}) as Record<string,unknown>;return <div key={String(row.market)}><span>{String(row.market)}</span><strong>{codeText(locale,row.status)}</strong><small>{metrics.tradeCount!=null?`${tx(locale,"minimumTrades")}: ${String(metrics.tradeCount)} · PF ${String(metrics.profitFactor??"—")} · Sharpe ${String(metrics.sharpe??"—")}`:new Date().toLocaleDateString(locale)}</small></div>})}</div></PageSection>;
}
...
function SettingsView({ locale, title, saveLabel, notifyLabel }: { locale:Locale; title:string; saveLabel:string; notifyLabel:string }) {
  type SettingsForm={locale:Locale;baseCurrency:string;paperCapital:string;riskPlan:RiskPlanId;emailAlerts:boolean;alertEmail:string;enabledMarkets:MarketCode[];marketLimits:Record<MarketCode,number>;riskBudgetPct:number;maxWeightPct:number;maxSectorPct:number;drawdownBreakerPct:number;allowMinimumLotException:boolean};
  const makeLimits=(plan:RiskPlanId,markets:MarketCode[])=>Object.fromEntries(MARKETS.map(item=>[item,Math.min(100,Math.max(RISK_PLANS[plan].maxMarketPct,Math.ceil(100/Math.max(1,markets.length))))])) as Record<MarketCode,number>;
  const [form,setForm]=useState<SettingsForm>({locale,baseCurrency:"TWD",paperCapital:"",riskPlan:"capital_first",emailAlerts:false,alertEmail:"",enabledMarkets:[...MARKETS],marketLimits:makeLimits("capital_first",[...MARKETS]),riskBudgetPct:RISK_PLANS.capital_first.riskBudgetPct,maxWeightPct:RISK_PLANS.capital_first.maxWeightPct,maxSectorPct:RISK_PLANS.capital_first.maxSectorPct,drawdownBreakerPct:RISK_PLANS.capital_first.drawdownBreakerPct,allowMinimumLotException:true});
  const [message,setMessage]=useState(""),[limitsTouched,setLimitsTouched]=useState(false);
  useEffect(()=>{fetch("/api/settings",{cache:"no-store"}).then(async r=>await r.json() as {settings?:Record<string,unknown>}).then(d=>{if(!d.settings)return;const policy=(d.settings.riskPolicy??{}) as Record<string,unknown>,plan=(String(d.settings.riskPlan??"capital_first") in RISK_PLANS?String(d.settings.riskPlan):"capital_first") as RiskPlanId,enabled=(Array.isArray(d.settings.enabledMarkets)?d.settings.enabledMarkets:MARKETS).filter(item=>MARKETS.includes(String(item) as MarketCode)) as MarketCode[],storedLimits=(d.settings.marketLimits??policy.marketLimits??{}) as Partial<Record<MarketCode,number>>,defaults=makeLimits(plan,enabled.length?enabled:[...MARKETS]);setForm({locale:(d.settings.locale as Locale)??locale,baseCurrency:String(d.settings.baseCurrency??"TWD"),paperCapital:d.settings.paperCapital?String(d.settings.paperCapital):"",riskPlan:plan,emailAlerts:Boolean(d.settings.emailAlerts),alertEmail:String(d.settings.alertEmail??""),enabledMarkets:enabled.length?enabled:[...MARKETS],marketLimits:Object.fromEntries(MARKETS.map(item=>[item,Number(storedLimits[item]??defaults[item])])) as Record<MarketCode,number>,riskBudgetPct:Number(policy.riskBudgetPct??RISK_PLANS[plan].riskBudgetPct),maxWeightPct:Number(policy.maxWeightPct??RISK_PLANS[plan].maxWeightPct),maxSectorPct:Number(policy.maxSectorPct??RISK_PLANS[plan].maxSectorPct),drawdownBreakerPct:Number(policy.drawdownBreakerPct??RISK_PLANS[plan].drawdownBreakerPct),allowMinimumLotException:policy.allowMinimumLotException!==false});setLimitsTouched(Boolean(policy.customized));});},[locale]);
  function resetTemplate(plan:RiskPlanId=form.riskPlan){const template=RISK_PLANS[plan],limits=makeLimits(plan,form.enabledMarkets);setForm({...form,riskPlan:plan,riskBudgetPct:template.riskBudgetPct,maxWeightPct:template.maxWeightPct,maxSectorPct:template.maxSectorPct,drawdownBreakerPct:template.drawdownBreakerPct,marketLimits:limits});setLimitsTouched(false);setMessage("");}
  function toggleMarket(market:MarketCode){const selected=form.enabledMarkets.includes(market)?form.enabledMarkets.filter(item=>item!==market):[...form.enabledMarkets,market];if(!selected.length){setMessage(tx(locale,"errorRiskPolicy",{rule:"RISK_MARKETS_REQUIRED"}));return;}const marketLimits=limitsTouched?form.marketLimits:makeLimits(form.riskPlan,selected);setForm({...form,enabledMarkets:selected,marketLimits});}
  async function save(){const riskPolicy={enabledMarkets:form.enabledMarkets,marketLimits:form.marketLimits,riskBudgetPct:form.riskBudgetPct,maxWeightPct:form.maxWeightPct,maxSectorPct:form.maxSectorPct,drawdownBreakerPct:form.drawdownBreakerPct,allowMinimumLotException:form.allowMinimumLotException,marketLimitsCustomized:limitsTouched};const r=await fetch("/api/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,paperCapital:Number(form.paperCapital)||null,riskPolicy})});const d=await r.json() as ApiErrorPayload&{portfolioAdjustment?:{capital:number;delta:number}};if(!r.ok){setMessage(apiErrorText(locale,d,"errorSettings"));return;}const adjustment=d.portfolioAdjustment;setMessage(adjustment?tx(locale,"capitalSynced",{capital:adjustment.capital.toLocaleString(locale),delta:`${adjustment.delta>0?"+":""}${adjustment.delta.toLocaleString(locale)}`}):tx(locale,"saved"));}
  ...
  const rangesValid=form.riskBudgetPct>=.1&&form.riskBudgetPct<=2&&form.maxWeightPct>=1&&form.maxWeightPct<=30&&form.maxSectorPct>=5&&form.maxSectorPct<=100&&form.drawdownBreakerPct>=5&&form.drawdownBreakerPct<=30&&form.enabledMarkets.every(item=>form.marketLimits[item]>=10&&form.marketLimits[item]<=100);
  const policyValid=rangesValid&&form.maxWeightPct<=form.maxSectorPct&&form.enabledMarkets.every(item=>form.maxWeightPct<=form.marketLimits[item]);
  const marketTotal=form.enabledMarkets.reduce((total,item)=>total+form.marketLimits[item],0),cashReserve=Math.max(0,100-marketTotal);
  return <PageSection title={title} eyebrow={tx(locale,"settingsEyebrow")}><div className="settings-card"><label>{tx(locale,"language")}<select value={form.locale} onChange={e=>setForm({...form,locale:e.target.value as Locale})}><option value="zh-TW">繁體中文</option><option value="zh-CN">简体中文</option><option value="en">English</option><option value="ja">日本語</option><option value="ko">한국어</option></select></label><label>{tx(locale,"baseCurrency")}<select value={form.baseCurrency} onChange={e=>setForm({...form,baseCurrency:e.target.value})}><option>TWD</option><option>USD</option><option>JPY</option><option>HKD</option><option>CNY</option><option>KRW</option><option>SGD</option></select></label><label>{tx(locale,"paperCapital")}<input inputMode="decimal" value={form.paperCapital} onChange={e=>setForm({...form,paperCapital:e.target.value})} placeholder="1000000"/></label><label>{tx(locale,"riskPlan")}<select value={form.riskPlan} onChange={e=>resetTemplate(e.target.value as RiskPlanId)}>{(Object.keys(RISK_PLANS) as RiskPlanId[]).map(id=><option key={id} value={id}>{riskPlanName(locale,id)} · {tx(locale,"singleTradeRisk")} {RISK_PLANS[id].riskBudgetPct}% · {tx(locale,"maxPosition")} {RISK_PLANS[id].maxWeightPct}%</option>)}</select></label><section className="settings-wide risk-settings"><header><div><h3>{tx(locale,"investmentMarkets")}</h3><p>{tx(locale,"investmentMarketsNote")}</p></div><button type="button" className="secondary" onClick={()=>resetTemplate()}>{tx(locale,"resetRiskTemplate")}</button></header><div className="market-toggle-grid">{MARKETS.map(item=><button type="button" key={item} className={form.enabledMarkets.includes(item)?"active":""} onClick={()=>toggleMarket(item)} aria-pressed={form.enabledMarkets.includes(item)}>{item}</button>)}</div><h3>{tx(locale,"customRiskPolicy")}</h3><div className="risk-input-grid"><label>{tx(locale,"singleTradeRisk")}<input type="number" min="0.1" max="2" step="0.1" value={form.riskBudgetPct} onChange={e=>setForm({...form,riskBudgetPct:Number(e.target.value)})}/></label><label>{tx(locale,"maxPosition")}<input type="number" min="1" max="30" step="1" value={form.maxWeightPct} onChange={e=>setForm({...form,maxWeightPct:Number(e.target.value)})}/></label><label>{tx(locale,"maxSector")}<input type="number" min="5" max="100" step="1" value={form.maxSectorPct} onChange={e=>setForm({...form,maxSectorPct:Number(e.target.value)})}/></label><label>{tx(locale,"drawdownBreaker")}<input type="number" min="5" max="30" step="1" value={form.drawdownBreakerPct} onChange={e=>setForm({...form,drawdownBreakerPct:Number(e.target.value)})}/></label>{form.enabledMarkets.map(item=><label key={item}>{tx(locale,"marketLimit",{market:item})}<input type="number" min="10" max="100" step="1" value={form.marketLimits[item]} onChange={e=>{setLimitsTouched(true);setForm({...form,marketLimits:{...form.marketLimits,[item]:Number(e.target.value)}});}}/></label>)}</div><p className={`risk-policy-preview ${policyValid?"":"conflict"}`}>{policyValid?tx(locale,"riskPolicyPreview",{position:form.maxWeightPct,sector:form.maxSectorPct,marketTotal,cash:cashReserve}):tx(locale,"riskPolicyConflictPreview")}</p><label className="check-row"><input type="checkbox" checked={form.allowMinimumLotException} onChange={e=>setForm({...form,allowMinimumLotException:e.target.checked})}/> {tx(locale,"allowMinimumLot")}</label><p className="risk-limit-note">{tx(locale,"minimumLotRule")}</p></section><label>{tx(locale,"email")}<input type="email" value={form.alertEmail} onChange={e=>setForm({...form,alertEmail:e.target.value})}/></label><label className="check-row"><input type="checkbox" checked={form.emailAlerts} onChange={e=>setForm({...form,emailAlerts:e.target.checked})}/> {tx(locale,"enableEmail")}</label><div className="settings-actions"><button onClick={save} disabled={!policyValid}>{saveLabel}</button><button className="secondary" onClick={notify}>{notifyLabel}</button><span>{message}</span></div></div></PageSection>;
}
