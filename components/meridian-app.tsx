      ...
      <main className="workspace">
        {(["dashboard", "scanner", "signals", "security"] as AppView[]).includes(view) && <>
          ...
          {view !== "security" && payload?.meta.scan && <section className="scan-audit-strip" aria-label={x("fullScan")}>
            ...
            <p>{x("universeTarget")} · {payload.meta.scan.completedAt ? new Date(payload.meta.scan.completedAt).toLocaleString(locale) : x("running")}</p>
          </section>}
...
function SettingsView({ locale, title, saveLabel, notifyLabel }: { locale:Locale; title:string; saveLabel:string; notifyLabel:string }) {
  ...
  async function save(){const r=await fetch("/api/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({...form,paperCapital:Number(form.paperCapital)||null})});setMessage(tx(locale,r.ok?"saved":"saveFailed"));}
