...
function PortfolioView({ locale, title, setup }: { locale:Locale; title:string; setup:string }) {
  ...
  return <PageSection title={title} eyebrow={x("portfolioEyebrow")}>
    ...
    <div className="portfolio-note"><strong>{x("positions")}: {positions.length}</strong><span>{x("feeNotice")}</span><button className="portfolio-refresh-button" disabled={refreshing||positions.length===0} onClick={()=>void refreshPositions()}>{refreshing?x("refreshingPositions"):x("refreshPositions")}</button><b>{message}</b></div>
    {positions.length?<div className="data-table portfolio-table"><table><thead><tr><th>{x("symbol")}</th><th>{x("market")}</th><th>{x("quantity")} / {x("sellable")}</th><th>{x("averagePrice")}</th><th>{x("quoteData")}</th><th>{x("baseValue")}</th><th>{x("pnl")}</th><th>{x("sell")}</th></tr></thead><tbody>{positions.map(row=>{const id=String(row.instrument_id),sellable=Number(row.sellable_quantity??0);return <tr key={id}><td><strong>{String(row.symbol)}</strong><small>{String(row.name)}</small></td><td>{String(row.market)}<small>{String(row.currency)}</small></td><td>{Number(row.quantity).toLocaleString(locale)}<small>{sellable.toLocaleString(locale)}</small></td><td>{Number(row.average_cost).toLocaleString(locale)}<small>{Number(row.price).toLocaleString(locale)}</small></td><td>{codeText(locale,row.freshness)}<small title={String(row.source??"")}>{row.captured_at?new Date(String(row.captured_at)).toLocaleString(locale):"—"}</small></td><td>{base} {Number(row.base_market_value).toLocaleString(locale)}</td><td className={Number(row.unrealized_pnl_base)>=0?"positive":"negative"}>{Number(row.unrealized_pnl_base).toLocaleString(locale)}</td><td><div className="sell-control"><input aria-label={`${x("sell")} ${String(row.symbol)}`} type="number" min="1" max={sellable} step="1" value={sellQty[id]??sellable} onChange={event=>setSellQty({...sellQty,[id]:Math.max(1,Number(event.target.value))})}/><button disabled={sellable<=0} onClick={()=>void sell(row)}>{x("sell")}</button></div></td></tr>})}</tbody></table></div>:<div className="empty-state">{x("positions")}: 0</div>}
